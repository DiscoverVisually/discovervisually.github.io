...
(function initGuidedSectionScroll() {
  const sections = [...document.querySelectorAll("main > section[id]")];
  if (sections.length < 2) return;

  const desktopPointer = window.matchMedia("(min-width: 1101px) and (hover: hover) and (pointer: fine)");
  const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)");
  const gestureThreshold = 38;
  let wheelDelta = 0;
  let resetTimer = 0;
  let unlockTimer = 0;
  ...
  function sectionTop(section) {
    ...
    return Math.max(0, Math.min(absoluteTop, maxScroll));
  }
  ...
  function currentSectionIndex() {
    ...
    sections.forEach((section, index) => {
      if (sectionTop(section) <= marker) current = index;
    });
    return current;
  }
  ...
  function moveToSection(direction) {
    ...
    const target = sectionTop(sections[targetIndex]);
    if (Math.abs(target - start) < 2) return false;

    const distance = Math.abs(target - start);
    const duration = Math.max(940, Math.min(1480, 860 + distance * 0.4));
  ...
  function handleWheel(event) {
    if (!desktopPointer.matches || reducedMotion.matches || event.ctrlKey || event.metaKey || event.shiftKey || isEditableTarget(event)) return;
    ...
    if (Math.abs(wheelDelta) < gestureThreshold) return;
    const gestureDirection = wheelDelta > 0 ? 1 : -1;
    wheelDelta = 0;
    window.clearTimeout(resetTimer);
    moveToSection(gestureDirection);
  }

  window.addEventListener("wheel", handleWheel, { passive: false });
  window.addEventListener("resize", clearAnimation, { passive: true });
